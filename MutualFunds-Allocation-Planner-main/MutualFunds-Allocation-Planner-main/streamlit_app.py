...
def main():
    ...
    if submit_button or plan is not None:
        ...
        # Retirement projection (uses existing engine chart)
        if plan.user_profile.monthly_investment > 0:
            retirement_age = st.slider(
                "Target retirement age",
                min_value=plan.user_profile.age + 1,
                max_value=75,
                value=min(60, 75),
                key="retirement_age_slider",
            )
            fig_proj = create_projection_chart(
                plan.user_profile.monthly_investment,
                plan.user_profile.age,
                retirement_age,
                plan.equity_percentage,
            )
            fig_proj.update_layout(template="plotly_dark")
